...
Detailed usage, architecture, performance notes, troubleshooting, and demo tooling.
For installation and the release overview, return to the [main README](../README.md).
...
The two install steps work around Chatterbox's package metadata pinning
`numpy<1.26`, which has no Python 3.12 wheel. Mynah runs on NumPy 2.x, and
`requirements.txt` lists Chatterbox's runtime dependencies directly. The
`--no-deps` flag prevents pip from trying to build the stale NumPy version.
`uv run` handles the same compatibility override through `pyproject.toml`.
...
See the [retest results and scope](platform-validation-2026-09-08.md).
...
![voices drawer](screenshot-voices.png)
...
## Legacy automated demo

The current launch demo is attached to the
[v1.0.1 release](https://github.com/seena18/mynah/releases/tag/v1.0.1). The
files and commands below reproduce the original v1.0.0 demo.
